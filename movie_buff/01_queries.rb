...
def harrison_ford
  ...
  # as a lead actor.
  Movie.joins(:actors).select(:id, :title).where("castings.ord > 1 AND actors.name = 'Harrison Ford'")
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:castings).group('movies.id').order('COUNT(castings.ord) DESC').limit(3)
