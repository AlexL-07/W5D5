...
def vanity_projects
  ...
  Movie.select(:id, :title, :name).joins(:actors).where('castings.ord = 1 AND movies.director_id = castings.actor_id')
...
def most_supportive
  ...
  Actor.select(:id, :name, 'COUNT(*) AS roles').joins(:castings).where('castings.ord > 1').order('roles DESC').group(:id).limit(2)
end